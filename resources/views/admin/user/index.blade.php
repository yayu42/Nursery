{{-- layouts/admin.blade.phpを読み込む --}}
@extends('layouts.admin')
@section('title', '職員一覧')

@section('content')
    <div class="container">
        <div class="row">
            <h2>職員一覧</h2>
        </div>
        <div class="row">
            <div class="col-md-4">
                <a href="{{ action('Admin\UserController@add') }}" role="button" class="btn btn-primary">新規作成</a>
            </div>
            <div class="col-md-8">
                <form action="{{ action('Admin\UserController@index') }}" method="get">
                    <div class="form-group row">
                        <label class="col-md-2">在籍職員</label>
                        <div class="col-md-8">
                            <input type="text" class="form-control" name="cond_title" value="{{ $cond_title }}">
                        </div>
                        <div class="col-md-2">
                            @csrf
                            <input type="submit" class="btn btn-primary" value="検索">
                        </div>
                    </div>
                </form>
            </div>
        </div>
        <div class="row">
            <div class="list-news col-md-12 mx-auto">
                <div class="row">
                    <table class="table table-dark">
                        <thead>
                            <tr>
                                <th width="5%">ID</th>
                                <th width="10%">名前</th>
                                <th width="10%">性別</th>
                                <th width="10%">役職</th>
                            </tr>
                        </thead>
                        <tbody>
                            @foreach($posts as $user)
                                <tr>
                                    <th>{{ $user->id }}</th>
                                    <td>{{ \Str::limit($user->name, 50) }}</td>
                                    <td>{{ \Str::limit($user->gender,40) }}</td>
                                    <yd>{{ \Str::limit($user->grade,50) }}</td>
                                    <td>
                                        <div>
                                            <a href="{{ action('Admin\UserController@edit', ['id' => $user->id]) }}">編集</a>
                                        </div>
                                        <div>
                                            <a href="{{ action('Admin\UserController@delete', ['id' => $user->id]) }}">削除</a>
                                        </div>
                                    </td>
                                </tr>
                            @endforeach
                        </tbody>
                    </table>
                </div>
            </div>
        </div>
    </div>
@endsection